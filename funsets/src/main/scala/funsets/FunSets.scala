...
object FunSets {
  ...
  /**
    ...
    * that satisfies `p`.
    */
  //  def exists(s: Set, p: Int => Boolean): Boolean = !forall(s, negP(p))
  def exists(s: Set, p: Int => Boolean): Boolean = !forall(s, (x: Int) => !p(x))

  // Is there a better or more concise (anonymous function) way of doing it?
  //  def negP(p: Int => Boolean)(x: Int): Boolean = {
  //    !p(x)
  //  }
